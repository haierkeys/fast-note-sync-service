...
var (
	...
	Success               = NewSuss(incr(1), lang{zh: "成功", en: "Success"})
	SuccessCreate         = NewSuss(incr(1), lang{zh: "创建成功", en: "Create Success"})
	SuccessUpdate         = NewSuss(incr(1), lang{zh: "更新成功", en: "Update Success"})
	SuccessDelete         = NewSuss(incr(1), lang{zh: "删除成功", en: "Delete Success"})
	...
	ErrorFileDeleteFailed          = NewError(incr(400), lang{zh: "附件删除失败", en: "Delete file failed"})
	ErrorFileModifyOrCreateFailed  = NewError(incr(400), lang{zh: "附件修改或创建失败", en: "File modify or create failed"})
	ErrorFileContentModifyFailed   = NewError(incr(400), lang{zh: "修改文件内容失败", en: "Modify file content failed"})
	ErrorFileGetFailed             = NewError(incr(400), lang{zh: "获取文件失败", en: "Get file failed"})
	ErrorFileUploadSessionNotFound = NewError(incr(400), lang{zh: "上传文件会话不存在", en: "Upload file session not found"})
	ErrorFileUploadFailed          = NewError(incr(400), lang{zh: "上传文件失败", en: "Upload file failed"})
	ErrorFileUploadCheckFailed     = NewError(incr(400), lang{zh: "上传文件失败检测失败", en: "File upload check failed"})
	ErrorFileListFailed            = NewError(incr(400), lang{zh: "文件列表获取失败", en: "File list get failed"})

	// --------- 笔记操作错误

	ErrorNoteDeleteFailed         = NewError(incr(400), lang{zh: "笔记删除失败", en: "Delete note failed"})
	ErrorNoteModifyOrCreateFailed = NewError(incr(400), lang{zh: "笔记修改或创建失败", en: "Note modify or create failed"})
	ErrorNoteContentModifyFailed  = NewError(incr(400), lang{zh: "修改笔记内容失败", en: "Modify note content failed"})
