...
- [x] **🤖 MCP サポート**：AI MCP 関連機能のサポートを追加。
- [ ] **より多くのデータベースタイプのサポート**
...
FNS は **MCP (Model Context Protocol)** をネイティブサポートするようになりました。

FNS を MCP サーバーとして Cherry Studio、Cursor などの互換性のある AI クライアントに直接接続できます。接続後、AI はプライベートノートや添付ファイルの読み書き能力を持ちます。また、MCP によって行われたすべての変更は、WebSocket を通じてリアルタイムで各デバイス端末に同期されます。

### 接続設定（SSE モード）

FNS は **SSE プロトコル** を通じて MCP インターフェースを提供します。一般的なパラメータは以下の通りです：
- **エンドポイント URL**：`http://<サーバーIPまたはドメイン>:<ポート>/api/mcp/sse`
- **認証ヘッダー**：`token: <API トークン>`（WebGUI の「API 設定をコピー」から取得）


#### 例：Cherry Studio

Cherry Studio の MCP Server 設定に以下の JSON ノードを追加してください。
*（注：`<ServerIP>`、`<Port>`、`<Token>` を実際の情報に置き換えてください）*
