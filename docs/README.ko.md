...
* **💻 Web 관리 패널**: 현대적인 관리 인터페이스를 내장하여 사용자 생성, 플러그인 설정 생성, 저장소 및 노트 내용 관리를 쉽게 수행할 수 있습니다.
* **🔄 멀티 디바이스 노트 동기화**:
    * **Vault (저장소)** 자동 생성을 지원합니다.
    * 노트 관리(추가, 삭제, 수정, 조회)를 지원하며, 변경 사항은 밀리초 단위로 모든 온라인 기기에 실시간으로 배포됩니다.
* **🖼️ 첨부 파일 동기화 지원**:
    * 이미지 등 비노트 파일 동기화를 완벽하게 지원합니다.
    * *(참고: 서버 v0.9+ 및 [Obsidian 플러그인 v1.0+](https://github.com/haierkeys/obsidian-fast-note-sync/releases) 가 필요하며, Obsidian 설정 파일 동기화는 지원하지 않습니다)*
* **📝 노트 히스토리**:
    * 웹 페이지 및 플러그인에서 각 노트의 과거 수정 버전을 확인할 수 있습니다.
    * (서버 v1.2+ 필요)
* **⚙️ 설정 동기화**:
...
- 이 플러그인이 유용하다고 생각하시고 개발 지속을 지원하고 싶으시다면 아래의 방법으로 후원해 주세요:

  | Ko-fi *중국 외 지역*  |  | 위챗(WeChat) 스캔 후원 *중국 지역* |
  | --- | ---| --- |
...
우리는 지속적으로 개선하고 있으며, 향후 개발 계획은 다음과 같습니다:

- [ ] **Git 버전 관리 통합**: 노트에 대해 보다 안전한 버전 추적 기능을 제공합니다.
- [ ] **동기화 알고리즘 최적화**: `google-diff-match-patch`를 통합하여 보다 효율적인 증분 동기화를 구현합니다.
- [ ] **클라우드 스토리지 및 백업 전략**:
    - [ ] 사용자 정의 백업 전략 구성.
    - [ ] 멀티 프로토콜 적응: S3 / Minio / Cloudflare R2 / Aliyun OSS / WebDAV.

> **개선 제안이나 새로운 아이디어가 있으시면 Issue를 통해 공유해 주세요. 적절한 제안을 신중하게 검토하고 반영하겠습니다.**
...
**스크립트의 주요 동작:**
  ...
  * 기본적으로 `/opt/fast-note`에 설치되며, `/usr/local/bin/fast-note`에 바로가기 명령어를 생성합니다.
...
기본 설정 파일은 `config.yaml`이며, 프로그램은 **루트 디렉토리** 또는 **config/** 디렉토리에서 자동으로 파일을 찾습니다.
