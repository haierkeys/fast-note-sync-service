...
// Migration001 修改 vault 表的 note_count 字段,去掉 NOT NULL 约束
...
// Up 执行升级
func (m *Migration001) Up(db *gorm.DB, ctx context.Context) error {

	global.Logger.Info("VaultMigration Up - Starting vault table migration")
	...
	`
	if err := svc.VaultMigrationQuery(createNewTable, true); err != nil {
	...
	copyData := `
		...
		FROM vault
	`
	if err := svc.VaultMigrationQuery(copyData); err != nil {
	...
	if err := svc.VaultMigrationQuery("DROP TABLE vault"); err != nil {
	...
	if err := svc.VaultMigrationQuery("ALTER TABLE vault_new RENAME TO vault"); err != nil {
	...
	if err := svc.VaultMigrationQuery(createIndex); err != nil {
		global.Logger.Error("Step 5 failed", zap.Error(err))
		return err
	}
	global.Logger.Info("Step 5: Index recreated successfully")

	global.Logger.Info("VaultMigration Up - Completed successfully")
	return nil
}
